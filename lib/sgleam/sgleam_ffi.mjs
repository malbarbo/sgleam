...
export function try_main(main, input_kind, show_output, repl_files) {
    if (repl_files.length !== 0) {
        if (!globalThis.repl_files) {
            globalThis.repl_files = new Set();
        }
        // Kept, not replaced: an fn from an earlier input still raises there.
        for (const file of repl_files) {
            globalThis.repl_files.add(file);
        }
    }
    try {
        let r;
...
function show_error(err) {
    if (err.gleam_error) {
        // A generated module has no location worth showing.
        if (globalThis.repl_files && globalThis.repl_files.has(err.file)) {
