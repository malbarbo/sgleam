...
class Worker {
  ...
  private makeSgleamEnv() {
    return {
      ...
      sgleam_draw_svg: (ptr: number, len: number): void => {
        ...
        this.channel.svg(new TextDecoder().decode(buf.slice(ptr, ptr + len)));
      },
      ...
      sgleam_text_height: (
        ...
        const buf = new Uint8Array(this.getBuffer());
        const jtext = new TextDecoder().decode(buf.slice(text, text + textLen));
        const jfont = new TextDecoder().decode(buf.slice(font, font + fontLen));
      ...
      fd_write: (
        ...
        try {
          ...
          for (let i = 0; i < iovsLen; i++) {
            ...
            const buf = new Uint8Array(this.getBuffer(), bufPtr, bufLen);
      ...
      args_get: (argvPtr: number, argvBuf: number): number => {
        try {
          ...
          for (let i = 0; i < argPointers.length; i++) {
            dataView.setInt32(argvPtr + i * 4, argPointers[i], true);
          }
  ...
  private async loadWasm(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        this.channel.error(`Error loading sgleam.wasm: ${response.status}`);
        return;
      }
      ...
      await this.instantiateWasm(await WebAssembly.compile(wasmBytes.buffer));
    } catch (error) {
      this.channel.error(`${error}`);
    }
  }

  private async instantiateWasm(wasmModule: WebAssembly.Module): Promise<void> {
  ...
  formatRepl(input: string): void {
    ...
    this.exports.string_deallocate(ptr);
    if (r !== 0) {
      this.channel.format(readCstr(this.exports, r));
